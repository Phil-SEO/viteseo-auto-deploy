...
show_next_steps() {
    ...
    echo ""
    echo -e "  1. ${CYAN}Configure GitHub Secrets${NC}"
    ...
    echo ""
    echo -e "  2. ${CYAN}Generate SSH keys (optional helper)${NC}"
    echo "     Run: ./deploy/setup-deployment.sh"
    echo ""
    echo -e "  3. ${CYAN}Set up your server${NC}"
    ...
    echo ""
    echo -e "  4. ${CYAN}Commit and push the new files${NC}"
    echo "     git add .github/workflows deploy"
    echo "     git commit -m 'Add auto-deployment workflow'"
    echo "     git push"
    echo ""
    echo -e "  5. ${CYAN}Test the deployment${NC}"
...
# Handle being piped to bash or run directly
# Use :- to provide empty default, avoiding "unbound variable" error with nounset
